...
//		dijkstra(g);
	...
	protected static void connectedComponents(IGraph g) {
		Clock clock = Clock.systemDefaultZone();
		GraphComputer computer = new GraphComputer();

		ConnectedComponents cc = new ConnectedComponents(g);
		long start = clock.millis();
		computer.execute(cc, g, 20);
		long end = clock.millis();
		System.out.println("Connected Components in "+(end - start) / 1000.0+" s");
		computer.dismiss();
		
		Int2IntMap nodeToComponent = computer.getResult(cc);
		int totComponents = new IntArraySet( nodeToComponent.values() ).size();
		System.out.println("Tot components: "+totComponents);
	}
	...
	protected static void pagerank(IGraph g) {
		...
		long end = clock.millis();
		System.out.println("PageRank in "+(end - start) / 1000.0+" s");
		computer.dismiss();
